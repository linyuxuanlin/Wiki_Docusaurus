...
IDD 表示的是 CMOS 电路中从漏极（D）到漏极（D）的电流（I），如果是 TTL 电路则称为 ICC（从集电极到集电极的电流）。Gross IDD 指的是流入 VDD 管脚的总电流，在 Wafer Probe 或成品阶段都可以进行测试。
...
![](https://cos.wiki-power.com/img/20220728155246.png)
...
IDD 静态测试代表在 DUT 在测试时未激活。举个例子，下表是一个 IDD 参数样本：
