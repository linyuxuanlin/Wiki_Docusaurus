...
VOL 表示低电平（L）输出（O）时的最高电压（V）限制（不会被识别成逻辑 1）。IOL 表示低电平（L）输出（O）时灌电流（I，sink）的驱动能力。它们共同衡量的是输出低电平时的阻抗，保证在适当输出的电压下能吸收恒定的电流值。

VOH 表示高电平（H）输出（O）时的最低电压（V）限制（不会被识别成逻辑 0）。IOL 表示高电平（H）输出（O）时拉电流（I，source）的驱动能力。它们共同衡量的是输出高电平时的阻抗，保证在适当输出的电压下能输出恒定的电流值。
...
对 VOL/IOL & VOH/IOH 的测试，主要是验证当施加拉或灌电流时，VOL/VOH 是否处于正确的电平。测试方法有静态法与动态法。静态法是用 PMU 对引脚施加电压和电流，再逐一测电压与电流？；动态法是在功能测试中加动态负载电流，再比较电压。

### VOH 与 IOH 测试（串行静态法）

使用串行静态法测量 VOH 与 IOH，实质测量的是引脚输出 Buffer 的电阻。其测试示意图如下：

![](https://cos.wiki-power.com/img/20220728143124.png)

测试流程如下：

1. 需要先通过预处理，将待测引脚输出设置为高电平
2. 使用 PMU 向引脚吸收恒定的 IOH，等待 1-5 毫秒（在 PMU 设 delay）
3. 将测得的 VOH 电压与标称值相比较，如果低于标称值则为不通过。

需要注意的事项：

- 因为 IOH 是从 DUT 流向 PMU，所以它是一个负值。
- 因为施加的是恒流，所以需要设置电压钳，如果测出的电压超出了钳位电压，有可能是引脚逻辑设成了低电平，触发了对地保护二极管正偏。
- VDDmin 参数表示能使 DUT 正常进行测试的最小供电电压，再小将无法得出准确的测试结果。

### VOL 与 IOL 测试（串行静态法）

使用串行静态法测量 VOL 与 IOL，测的是引脚输入 Buffer 的电阻，其测试示意图如下：

![](https://cos.wiki-power.com/img/20220728150542.png)

测试流程如下：

1. 需要先通过预处理，将待测引脚输出设置为低电平。
2. 使用 PMU 向引脚施加恒定的 IOH，等待 1-5 毫秒（在 PMU 设 delay）。
3. 将测得的 VOL 电压与标称值相比较，如果高于标称值则为不通过。

需要注意的事项：

- 因为 IOL 是从 PMU 流向 DUT，所以它是一个正电流值。
- 因为施加的是恒流，所以需要设置电压钳，如果测出电压低于钳位电压，有可能是逻辑设成了高电平，触发了对电源保护二极管正偏。
- VDDmin 参数表示能使 DUT 正常进行测试的最小供电电压，再小将无法得出准确的测试结果。

## IIL 与 IIH

IIL 指的是引脚被拉低（L）时输入（I）电流（I）的大小；IIH 指的是引脚被拉高（H）时输入（I）电流（I）的大小。举个例子，下表是 256 x 4 Static RAM 的 IIL 和 IIH 参数：

| Parameter | Description        | Test Conditions        | Min | Max | Units |
| --------- | ------------------ | ---------------------- | --- | --- | ----- |
| IIL, IIH  | Input Load Current | Vss ≤ Vin ≤ VDD(5.25V) | -10 | +10 | µA    |

IIL 用于衡量的是输入引脚到 VDD 的电阻值；IIH 衡量输入引脚到 VSS 的电阻值。该测试用于确保输入电阻满足设计需求、输入电流不会超标。IIL 与 IIH 可通过以下几种方法进行测试：
...
使用串行方法测试输入引脚 IIL 的示意图如下：

![](https://cos.wiki-power.com/img/20220729100620.png)

1. 首先要供 VDDmax 的电源给 DUT。
2. 将 DUT 所有输入引脚设高电平（VIH）。
3. 使用 PMU 将单个输入引脚拉低到地。
4. 等待 1~5 微秒，将检测的电流与标称值比较。如果小于 -10µA（电流能够灌进 DUT）即视为不通过。

### IIH 输入漏电流测试（串行法）

使用串行方法测试输入引脚 IIH 的示意图如下：
...
4. 等待 1~5 微秒，将检测的电流与标称值比较。如果高于 +10µA（电流能够流出 DUT）即视为不通过。

### IIL/IIH 输入漏电流测试（并行法）

在一些测试系统上，能对漏电流进行并行测量（Parallel Test Method）。并行测漏电流是用多个 PMU 对多个 pin 分别进行测量，所有输入引脚都被强制拉高，并且并行（同时）测量每个引脚的电流，随后将测试结果与标称值做比较得出结论。
...
指的是将 PMU 连接到所有输入引脚，测漏电流的总和。所有输入引脚强制拉高测总的漏电流，再强制拉低测总漏电流，再与标称值对比得出结论。
...
测试上下拉阻性输入是为了 **验证输入 Buffer 是否有正确的电源或接地电阻路径**。
...
高阻电流 IOZ 指的是输出引脚（O）高阻态（Z）下的漏电流（I）。IOZL 是指引脚低电平（L）状态时的漏电流；IOZH 是指高电平（H）状态时的漏电流。

测试高阻输出漏电流是为了 **确保双向 / 高阻输出引脚能正常输出高阻态（关断）**。IOZL 测的是输出高阻状态时，引脚对 VDD 的阻值；IOZH 测的是引脚对地的阻值。通常在规格书内是这么表示的：
...
串行静态测试 IOZL/IOZH，其流程如下：

1. 首先需要给器件供 VDD 的电源。
2. 将器件引脚预处理为高阻状态，使用 PMU 强制将引脚拉高 / 拉低。
3. 测量电流值，与标称值作比较，得出结论。

串行测试的优点是可以准确测量单个引脚的电流值，缺点是慢。此测试需要加电流钳。
...
1. 首先要确保这是个 TTL 器件的输入引脚。
2. 然后供 VCCmin 的电源。
3. 在设置了电压钳后，使用 PMU 施加 -15mA~-20mA 的电流（抽取电流）。
4. 测量得到引脚上的电压值，与标称值相对比（通常为 -1.5V）。

## 短路输出电流（IOS）

短路输出电流表示的是当输出引脚（O）在短路条件（S）下产生的电流（I）。目的是衡量当引脚输出高电平，但被短路至零电压时的电阻量，确保在最坏的负载条件下，电流值也能达标；也表示了 DUT 引脚可提供容性负载充电的最大瞬时电流，可据此计算上升时间。IOS 在规格书中是这样表示的：
...
1. 供 VDDmax 的电，对器件预处理，使得引脚输出高电平。
