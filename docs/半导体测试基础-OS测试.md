...
OS 测试串行静态法，实际上就是用 PMU 灌入电流测电压，因为这个电流会让上下某个保护二极管发生正偏，所以可以通过检测正偏压降来测出开短路异常。施加正电流使对电源二极管发生正偏的测试示意图如下：
...
3. 检测引脚电压
   - 高于 GT（+1.5V）：Fail（Open）
   - 低于 LT（+0.2V）：Fail（Short）
   - 正常区间（正偏后的压降约 0.65V）：Pass

施加负电流使对电源二极管发生正偏的测试示意图如下：
...
2. PMU 给引脚施加电流（约 -100µA）。
3. 检测引脚电压
   - 高于 GT（-0.2V）：Fail（Short）
   - 低于 LT（-1.5V）：Fail（Open）
   - 正常区间（正偏后的压降约 -0.65V）：Pass
