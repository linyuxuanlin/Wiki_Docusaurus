...
https://www.edn.com/optocoupler-simplifies-power-line-monitoring/

1. 解释是可以 AC ±24V 输入（差分信号）
2. 担心削波可能会在箝位二极管中消耗大量电流，因为没有输入隔离，这可能会扭曲10VAC电机和24VAC电磁阀的信号？
3. 串联的输入二极管，以防止在交流信号上产生低阻抗路径？
4. 用LM358B代替TLC2272来节省空间和成本
5. 连接3个ADC的所有4个输入，使之成为12个输入
6. 我们应该至少在2个输入端实现上述的电压转换和隔离水平。4个将是好的。全部都会很好。
   1. 我认为对于剩下的多达10个输入，我们可以使用一个没有输入运算放大器的交流光耦合器来消除分频电压。我们可以为24VAC开/关通道使用任何交流光耦合器，因为线性度和速度并不关键。